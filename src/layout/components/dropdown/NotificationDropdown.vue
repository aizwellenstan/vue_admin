    ...
    <div class="notification-dropdown__content pl-3 pr-3 pt-2 pb-2">
      ...
      >
        ...
        <span class="ellipsis" style="max-width: 85%;">
          <span v-if="option.details.name" class="text--bold">{{ option.details.name }}</span> {{ $t(`notifications.${option.name}`, { type: option.details.type }) }}
        </span>
      </div>
      <div class="row justify--space-between">
        <va-button class="ma-0 mb-2 mt-1" small>{{ $t('notifications.all') }}</va-button>
        <va-button class="ma-0 mb-2 mt-1" small outline :disabled="allRead" @click="markAllAsRead">{{ $t('notifications.mark_as_read') }}</va-button>
      </div>
    </div>
  </va-dropdown>
  </div>
